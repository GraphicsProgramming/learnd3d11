...
@(private)
BindVertexStageConstantBuffer :: proc (pipeline : ^Pipeline, slot_index : u32, buffer : ^d3d.IBuffer) {
	resource_desc := (ResourceDescriptor) {
		slot_index = slot_index,
		resource_stage = .VertexStage,
		resource_type = .Buffer,
	}

	pipeline.resources[resource_desc] = (^d3d.IDeviceChild)(buffer)
}
